  ...
  public void visitNode(Tree tree) {
    ...
    for (TypeTree typeTree : thrownList) {
      ...
      if (!reported.contains(fullyQualifiedName)) {
        String isSubtypeOfAny = isSubclassOfAny(symbolType, thrownList);
        if (isSubtypeOfAny != null) {
          addIssue(typeTree, "Remove the declaration of thrown exception '" + fullyQualifiedName + "' which is a subclass of '" + isSubtypeOfAny + "'.");
